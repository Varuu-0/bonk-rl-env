...
describe('normalizeMap', () => {
  describe('internal MapDef pass-through', () => {
    ...
    it('preserves authored spawnPoints/capZones/joints for bodies: [null] (#273)', () => {
      ...
      expect(out.capZones).toEqual([{ index: 0, owner: '', type: 1, fixture: 'plat_0', shapeType: 'rect' }]);
      expect(out.joints).toEqual([{ type: 'rv', name: 'j0', bodyA: 'plat_0', bodyB: 'plat_1' }]);
      expect(out.bodies).toEqual([]);
    });
  });
  ...
  describe('exported bonk format detection', () => {
    ...
    it('retains native shape transforms for rendering flattened fixtures', () => {
      const out = normalizeMap({
        physicsBodies: [{ position: { x: 30, y: -500 }, angle: 0.25 }],
        physicsShapes: [
          {
            type: 'po',
            center: { x: 4, y: -2 },
    ...
    it('folds rotated-rect world AABB extents into deathCenter (#420)', () => {
      ...
      expect(out.physics.deathCenter.y).toBeCloseTo((-410 + 400 + orientedHalf) / 2, 3);
    });

    it('degenerates to the axis-aligned rect bbox at angle zero (#420 regression guard)', () => {
      const out = normalizeMap({
        bodies: [
          { bodyIndex: 0, name: 'anchor', type: 'rect', x: 0, y: -400, width: 200, height: 20, static: true },
          { bodyIndex: 1, name: 'ramp', type: 'rect', x: 0, y: 400, width: 600, height: 40, static: true },
        ],
        spawns: [
          { x: -100, y: -380, blue: true },
          { x: 100, y: -380, red: true },
      ...
      // Unrotated extents give y bounds [-410..420] → center exactly 5;
      // θ=0 must keep producing the identical values so unrotated maps
      // are unaffected.
      expect(out.physics.deathCenter).toEqual({ x: 0, y: 5 });
    });

    it('keeps an origin-symmetric rotated rect centered at {0,0} (#420)', () => {
      const out = normalizeMap({
        bodies: [
          {
            bodyIndex: 0,
    ...
    it('keeps fixture aliases unique and resolves joints to the first fixture of each native body (#307)', () => {
            ...
            fixtures: [
              {
                fixtureIndex: 0,
                name: 'Unnamed Shape',
                shape: { type: 'bx', typeName: 'rect', center: { x: -50, y: 0 }, angle: 0, width: 20, height: 10 },
              },
              {
                fixtureIndex: 1,
                name: 'Unnamed Shape',
            ...
            fixtures: [
              {
                fixtureIndex: 2,
                name: 'Unnamed Shape',
                shape: { type: 'bx', typeName: 'rect', center: { x: 0, y: 0 }, angle: 0, width: 200, height: 10 },
              },
            ],
          },
        ],
      ...
      expect(out.bodies.map((body: any) => body.name)).toEqual(['compound#0', 'compound#1', 'anchor']);
  ...
  describe('native/structured export hardening (#307 review)', () => {
            ...
            fixtures: [
              {
                fixtureIndex: 0,
                name: 'Unnamed Shape',
                shape: { type: 'bx', typeName: 'rect', center: { x: -40, y: 0 }, angle: 0, width: 20, height: 10 },
              },
              {
                fixtureIndex: 1,
                name: 'Unnamed Shape',
        ...
        physicsBodies: [
          {
            ...
            fixtures: [
              null,
              {
                fixtureIndex: 7,
                name: 'Unnamed Shape',
                shape: { type: 'bx', typeName: 'rect', center: { x: 0, y: 0 }, angle: 0, width: 10, height: 10 },
              },
            ],
          },
        ],
        ...
        physicsBodies: [
          {
            ...
            typeName: 'static',
            position: { x: 0, y: 0 },
            fixtures: [null, { name: 'Unnamed Shape' }],
          },
        ],
        physicsFixtures: [
              ...
              fixtures: [
                {
                  fixtureIndex: 0,
                  name: 'Unnamed Shape',
                  shape: { type: 'xx', typeName: 'weird', center: { x: 0, y: 0 }, angle: 0 },
                },
              ],
            },
          ],
            ...
            fixtures: [
              {
                fixtureIndex: 0,
                name: 'Unnamed Shape',
                shape: { type: 'bx', typeName: 'rect', center: { x: 100, y: 0 }, angle: 0, width: 20, height: 10 },
              },
            ],
          },
        ],
    ...
    it('preserves exported bounds in map-pixel units for downstream conversion (#320)', () => {
      const out = normalizeMap({
        ...
        bodies: [{ bodyIndex: 0, name: 'floor', type: 'rect', x: 0, y: 200, width: 900, height: 20, static: true }],
        spawns: [
