...
describe('config-loader env vars and CLI', () => {
  ...
  const envKeys = [
    ...
    'TEST_MODE',
    'AI_PLAYER_ID',
    'RANDOM_OPP_MOVE_PROB',
    'RANDOM_OPP_UP_PROB',
    'RANDOM_OPP_DOWN_PROB',
    'RANDOM_OPP_HEAVY_PROB',
    'RANDOM_OPP_GRAPPLE_PROB',
    'FRAME_SKIP',
    'MAX_TICKS',
    'NUM_OPPONENTS',
    'RANDOM_OPPONENT',
  ...
  describe('environment variable overrides', () => {
    ...
    it('FRAME_SKIP overrides the default frame skip (#413)', () => {
      process.env.FRAME_SKIP = '4';
      const cfg = loadConfig(testDir);
      expect(cfg.environment.frameSkip).toBe(4);
    });

    it('FRAME_SKIP accepts the upper bound of the [1,100] contract', () => {
      process.env.FRAME_SKIP = '100';
      expect(loadConfig(testDir).environment.frameSkip).toBe(100);
    });

    it('invalid FRAME_SKIP (out of range / fractional / garbage) falls back to default', () => {
      process.env.FRAME_SKIP = '101';
      expect(loadConfig(testDir).environment.frameSkip).toBe(1);

      resetConfig();
      process.env.FRAME_SKIP = '0';
      expect(loadConfig(testDir).environment.frameSkip).toBe(1);

      resetConfig();
      process.env.FRAME_SKIP = '-2';
      expect(loadConfig(testDir).environment.frameSkip).toBe(1);

      resetConfig();
      process.env.FRAME_SKIP = '1.5';
      expect(loadConfig(testDir).environment.frameSkip).toBe(1);

      resetConfig();
      process.env.FRAME_SKIP = '4abc';
      expect(loadConfig(testDir).environment.frameSkip).toBe(1);
    });

    it('MAX_TICKS overrides the default episode length (#413)', () => {
      process.env.MAX_TICKS = '1800';
      const cfg = loadConfig(testDir);
      expect(cfg.environment.maxTicks).toBe(1800);
    });

    it('MAX_TICKS accepts the minimum positive length', () => {
      process.env.MAX_TICKS = '1';
      expect(loadConfig(testDir).environment.maxTicks).toBe(1);
    });

    it('invalid MAX_TICKS (zero / negative / fractional / garbage) falls back to default', () => {
      process.env.MAX_TICKS = '0';
      expect(loadConfig(testDir).environment.maxTicks).toBe(900);

      resetConfig();
      process.env.MAX_TICKS = '-900';
      expect(loadConfig(testDir).environment.maxTicks).toBe(900);

      resetConfig();
      process.env.MAX_TICKS = '900.5';
      expect(loadConfig(testDir).environment.maxTicks).toBe(900);

      resetConfig();
      process.env.MAX_TICKS = 'many';
      expect(loadConfig(testDir).environment.maxTicks).toBe(900);
    });
    ...
    it('NUM_OPPONENTS accepts the full documented 0-7 range', () => {
      process.env.NUM_OPPONENTS = '0';
      expect(loadConfig(testDir).environment.numOpponents).toBe(0);

      resetConfig();
      process.env.NUM_OPPONENTS = '7';
      expect(loadConfig(testDir).environment.numOpponents).toBe(7);
    });

    it('invalid NUM_OPPONENTS (out of range / negative / garbage) falls back to default', () => {
      process.env.NUM_OPPONENTS = '8';
      expect(loadConfig(testDir).environment.numOpponents).toBe(1);

      resetConfig();
      process.env.NUM_OPPONENTS = '-1';
      expect(loadConfig(testDir).environment.numOpponents).toBe(1);

      resetConfig();
      process.env.NUM_OPPONENTS = 'three';
      expect(loadConfig(testDir).environment.numOpponents).toBe(1);
    });

    it('RANDOM_OPPONENT=false disables the random opponent policy (#413)', () => {
      process.env.RANDOM_OPPONENT = 'false';
      const cfg = loadConfig(testDir);
      expect(cfg.environment.randomOpponent).toBe(false);
    });

    it('RANDOM_OPPONENT accepts the documented boolean spellings', () => {
      for (const truthy of ['true', '1', 'yes']) {
        resetConfig();
        process.env.RANDOM_OPPONENT = truthy;
        expect(loadConfig(testDir).environment.randomOpponent).toBe(true);
      }
      for (const falsy of ['false', '0', 'no']) {
        resetConfig();
        process.env.RANDOM_OPPONENT = falsy;
        expect(loadConfig(testDir).environment.randomOpponent).toBe(false);
      }
    });

    it('invalid RANDOM_OPPONENT value leaves the default policy enabled', () => {
  ...
  describe('CLI flag overrides', () => {
    ...
    it('all five --random-opp-*-prob CLI flags set their probabilities', () => {
      process.argv = [
        'node',
        'script.js',
        '--random-opp-move-prob',
        '0.91',
        '--random-opp-up-prob',
        '0.82',
        '--random-opp-down-prob',
        '0.73',
        '--random-opp-heavy-prob',
        '0.64',
        '--random-opp-grapple-prob',
        '0.55',
      ];
      const cfg = loadConfig(testDir);
      expect(cfg.environment.randomOppMoveProb).toBe(0.91);
    ...
    it('--kill-reward/--death-penalty/--time-penalty set the reward weights', () => {
      process.argv = ['node', 'script.js', '--kill-reward', '10', '--death-penalty', '-5', '--time-penalty', '-0.5'];
      const cfg = loadConfig(testDir);
      expect(cfg.reward.killReward).toBe(10);
      expect(cfg.reward.deathPenalty).toBe(-5);
      expect(cfg.reward.timePenalty).toBe(-0.5);
    ...
    it('missing values for the new environment-section flags are ignored', () => {
      process.argv = ['node', 'script.js', '--frame-skip', '--max-ticks'];
      let cfg = loadConfig(testDir);
      expect(cfg.environment.frameSkip).toBe(1);
      expect(cfg.environment.maxTicks).toBe(900);

      resetConfig();
      process.argv = ['node', 'script.js', '--num-opponents', '--random-opponent'];
      cfg = loadConfig(testDir);
      expect(cfg.environment.numOpponents).toBe(1);
      // The trailing bare --random-opponent flag still applies.
      expect(cfg.environment.randomOpponent).toBe(true);

      resetConfig();
      process.argv = ['node', 'script.js', '--default-map-path'];
      expect(loadConfig(testDir).environment.defaultMapPath).toBe('maps/bonk_WDB__No_Mapshake__716916.json');
    });
    ...
    it('--frame-skip sets the frame skip (#413)', () => {
      process.argv = ['node', 'script.js', '--frame-skip', '4'];
      const cfg = loadConfig(testDir);
      expect(cfg.environment.frameSkip).toBe(4);
    });

    it('--frame-skip rejects values outside [1,100] and non-integers', () => {
      process.argv = ['node', 'script.js', '--frame-skip', '101'];
      expect(loadConfig(testDir).environment.frameSkip).toBe(1);

      resetConfig();
      process.argv = ['node', 'script.js', '--frame-skip', '0'];
      expect(loadConfig(testDir).environment.frameSkip).toBe(1);

      resetConfig();
      process.argv = ['node', 'script.js', '--frame-skip', '2.5'];
      expect(loadConfig(testDir).environment.frameSkip).toBe(1);

      resetConfig();
      process.argv = ['node', 'script.js', '--frame-skip', 'abc'];
      expect(loadConfig(testDir).environment.frameSkip).toBe(1);
    });

    it('--max-ticks sets the episode length (#413)', () => {
      process.argv = ['node', 'script.js', '--max-ticks', '1800'];
      const cfg = loadConfig(testDir);
      expect(cfg.environment.maxTicks).toBe(1800);
    });

    it('--max-ticks rejects non-positive and fractional values', () => {
      process.argv = ['node', 'script.js', '--max-ticks', '0'];
      expect(loadConfig(testDir).environment.maxTicks).toBe(900);

      resetConfig();
      process.argv = ['node', 'script.js', '--max-ticks', '-5'];
      expect(loadConfig(testDir).environment.maxTicks).toBe(900);
    ...
    it('--num-opponents sets the opponent count (#413)', () => {
      process.argv = ['node', 'script.js', '--num-opponents', '3'];
      const cfg = loadConfig(testDir);
      expect(cfg.environment.numOpponents).toBe(3);
    });

    it('--num-opponents rejects values outside the documented 0-7 range', () => {
      process.argv = ['node', 'script.js', '--num-opponents', '8'];
      expect(loadConfig(testDir).environment.numOpponents).toBe(1);

      resetConfig();
      process.argv = ['node', 'script.js', '--num-opponents', '-1'];
      expect(loadConfig(testDir).environment.numOpponents).toBe(1);
    });

    it('--random-opponent (bare flag) keeps the random policy enabled', () => {
      process.argv = ['node', 'script.js', '--random-opponent'];
      const cfg = loadConfig(testDir);
      expect(cfg.environment.randomOpponent).toBe(true);
    });

    it('--random-opponent consumes an explicit boolean value (#413)', () => {
      process.argv = ['node', 'script.js', '--random-opponent', 'false'];
      expect(loadConfig(testDir).environment.randomOpponent).toBe(false);
    ...
    it('a bare --random-opponent does not swallow a following flag or value', () => {
      process.argv = ['node', 'script.js', '--random-opponent', '--seed', '7'];
      const cfg = loadConfig(testDir);
      expect(cfg.environment.randomOpponent).toBe(true);
      expect(cfg.environment.seed).toBe(7);

      resetConfig();
      process.argv = ['node', 'script.js', '--random-opponent', 'unrecognized'];
      expect(loadConfig(testDir).environment.randomOpponent).toBe(true);
    });

    it('--no-random-opponent disables the random opponent policy', () => {
      process.argv = ['node', 'script.js', '--no-random-opponent'];
      const cfg = loadConfig(testDir);
      expect(cfg.environment.randomOpponent).toBe(false);
    });

    it('--default-map-path sets the default map path (#413)', () => {
      process.argv = ['node', 'script.js', '--default-map-path', 'maps/custom.json'];
      const cfg = loadConfig(testDir);
      expect(cfg.environment.defaultMapPath).toBe('maps/custom.json');
    });

    it('--default-map-path and --map are aliases of the same setting', () => {
      process.argv = ['node', 'script.js', '--map', 'maps/via-map.json', '--default-map-path', 'maps/via-doc.json'];
      const cfg = loadConfig(testDir);
      expect(cfg.environment.defaultMapPath).toBe('maps/via-doc.json');
    });
  ...
  describe('priority order', () => {
    ...
    it('environment knobs: defaults < config.json < env vars (#413)', () => {
      fs.writeFileSync(
        configPath,
        JSON.stringify({
          environment: { maxTicks: 1200, numOpponents: 2 },
        }),
      );
      process.env.MAX_TICKS = '1500';
      const cfg = loadConfig(testDir);
      expect(cfg.environment.maxTicks).toBe(1500);
      expect(cfg.environment.numOpponents).toBe(2);
    });

    it('environment knobs: env vars < CLI flags (#413)', () => {
      process.env.FRAME_SKIP = '2';
      process.env.MAX_TICKS = '1500';
      process.env.NUM_OPPONENTS = '2';
      process.env.RANDOM_OPPONENT = 'false';
      process.env.DEFAULT_MAP_PATH = 'maps/env.json';
      process.argv = [
        'node',
        'script.js',
        '--frame-skip',
  ...
  describe('resolveEnvironmentConfig', () => {
    ...
    it('propagates the documented environment-section env vars (#413)', () => {
      process.env.FRAME_SKIP = '4';
      process.env.MAX_TICKS = '1800';
      process.env.NUM_OPPONENTS = '3';
      process.env.RANDOM_OPPONENT = 'false';
      ...
      expect(merged.maxTicks).toBe(1800);
      expect(merged.numOpponents).toBe(3);
      expect(merged.randomOpponent).toBe(false);
    });
