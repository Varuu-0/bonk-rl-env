...
describe('BonkEnvironment numOpponents validation (#392)', () => {
  ...
  it(`rejects one opponent past the MAX_OPPONENTS bound`, () => {
    const message = caughtMessage(() => new BonkEnvironment({ numOpponents: MAX_OPPONENTS + 1, maxTicks: 5, seed: 1 }));
    expect(message).toMatch(
  ...
  it('rejects the snake_case num_opponents alias above the bound', () => {
    const message = caughtMessage(() => new BonkEnvironment({ num_opponents: MAX_OPPONENTS + 1 } as any));
    expect(message).toMatch(
      new RegExp(`Invalid numOpponents ${MAX_OPPONENTS + 1}: expected at most ${MAX_OPPONENTS} opponents`),
    );
  });
  ...
  it('mentions the bundled Box2D pair-table capacity in the error', () => {
    const message = caughtMessage(() => new BonkEnvironment({ numOpponents: MAX_OPPONENTS + 1 }));
  ...
  it('keeps in-range normalization behavior (flooring, defaults)', () => {
    ...
    // Non-finite floats are not misconfigurations: they take the
    // documented default of 1 exactly like NaN (#398 review alignment).
    expect(SharedMemoryManager.normalizeNumOpponents(Infinity)).toBe(1);
    expect(SharedMemoryManager.normalizeNumOpponents(-Infinity)).toBe(1);
  });

  it('normalizeNumOpponents itself rejects out-of-range values with the labeled error', () => {
    expect(() => SharedMemoryManager.normalizeNumOpponents(MAX_OPPONENTS + 1)).toThrow(/Invalid numOpponents/);
    expect(() => SharedMemoryManager.floatsPerEnv(1e9)).toThrow(/Invalid numOpponents/);
  });
