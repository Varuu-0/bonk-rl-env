...
if (!hasWdb) {
  console.warn(
    `[MapIntegration] Optional WDB fixture ${MAP_FILES.wdb} is unavailable; WDB-specific tests are skipped.`,
  );
}
const simulationMapKeys = hasWdb ? (['simple1v1', 'wdb', 'weird'] as const) : (['simple1v1', 'weird'] as const);
...
describe('MapIntegration', () => {
  ...
  describe('map loading', () => {
    ...
    describeWdb('WDB', () => {
      ...
      it('has collides-none bodies (barriers)', () => {
        const map = loadMap(MAP_FILES.wdb);
        const collidesNone = map.bodies.filter(
          (b: any) => b.collides && !b.collides.g1 && !b.collides.g2 && !b.collides.g3 && !b.collides.g4,
        );
      ...
      it('has g1-only bodies', () => {
        const map = loadMap(MAP_FILES.wdb);
        const collidesG1Only = map.bodies.filter(
          (b: any) => b.collides && b.collides.g1 && !b.collides.g2 && !b.collides.g3 && !b.collides.g4,
        );
  ...
  describe('map body structure', () => {
    ...
    const mapKeys: (keyof typeof MAP_FILES)[] = hasWdb ? ['simple1v1', 'weird', 'wdb'] : ['simple1v1', 'weird'];
    ...
    it.each(mapKeys)('%s bodies have required fields', (key) => {
      const map = loadMap(MAP_FILES[key]);
      const allRequired = map.bodies.every(
        (b: any) =>
  ...
  describe('simulations', () => {
    ...
    describe('WeiRd DeAth BalL', () => {
      ...
      it('normalized collidesGroupN wires body maskBits via the engine filter', () => {
        ...
        const ghost = map.bodies.find(
          (b: any) => b.collides && !b.collides.g1 && !b.collides.g2 && !b.collides.g3 && !b.collides.g4,
        );
        const g1 = map.bodies.find((b: any) => b.collides && b.collides.g1 && (!b.collides.g3 || !b.collides.g4));
    ...
    describe('cross-map stability', () => {
      it.each(simulationMapKeys)('%s 60-tick simulation completes', (key) => {
  ...
  describe('out-of-bounds death circle follows the map center', () => {
    ...
    ] as const)('%s keeps the 850-unit OOB boundary centered on the authored geometry (#332)', (_name, shape) => {
    ...
    it('rotated rect contributes its oriented world AABB to deathCenter and the kill circle follows (#420)', () => {
      // Same exporter round-trip as the #332 polygon test, but for the
      // rect branch: addBody() builds rects as ORIENTED boxes rotated by
      // def.angle, so the 45° 600x40 ramp at (400, 400) spans
      // [173.726..626.274] on both axes (half-extent 320/√2 ≈ 226.274),
      // not [380..420]. Folding unrotated extents displaced the derived
      // center to y=5 instead of y≈108.137 (~103 px), shifting the whole
      // 850-unit OOB circle.
      const rawMap = extractMap({
        physics: {
          ppm: 30,
          bodies: [
            { p: [400, -400], a: 0, fx: [0], s: { type: 's', n: 'anchor' } },
            { p: [400, 400], a: 0, fx: [1], s: { type: 's', n: 'ramp' } },
          ],
          fixtures: [
            { sh: 0, n: 'anchor' },
            { sh: 1, n: 'ramp' },
          ...
          shapes: [
            { type: 'bx', c: [0, 0], a: 0, w: 200, h: 20 },
            { type: 'bx', c: [0, 0], a: Math.PI / 4, w: 600, h: 40 },
          ],
        },
        discs: [{ team: 1, x: 0, y: -150 }],
      } as any);
      ...
      const normalized = normalizeMap(rawMap);
      const dc: any = normalized.physics?.deathCenter;
      expect(dc.x).toBe(400);
      expect(dc.y).toBeCloseTo(108.13708498984761, 3);

      // The engine-built fixture must agree with the estimate: the ramp's
      // world vertices span exactly the oriented AABB the center was
      // derived from.
      const e = new PhysicsEngine();
      try {
        addAllBodies(e, normalized);
        const ramp = e.getBodyMap().get('ramp') as any;
        expect(ramp).toBeTruthy();
        const shape = ramp.GetShapeList();
        const verts = shape.m_vertices || [];
        const ys: number[] = [];
        for (let i = 0; i < shape.m_vertexCount; i++) {
          ys.push(ramp.GetWorldPoint(verts[i]).y * SCALE);
        }
        const orientedHalf = 320 / Math.SQRT2;
        expect(Math.min(...ys)).toBeCloseTo(400 - orientedHalf, 1);
        expect(Math.max(...ys)).toBeCloseTo(400 + orientedHalf, 1);
      } finally {
        safeDestroy(e);
      }

      // The environment installs the corrected center, and a disc in the
      // gap between the buggy southern boundary (5 - 850 = -845) and the
      // true boundary (108.137 - 850 ≈ -741.863) dies with native death
      // type 4 — under the regressed value that same disc would survive.
      const env = new BonkEnvironment({
        mapData: rawMap as any,
        numOpponents: 0,
        randomOpponent: false,
      ...
      try {
        expect((env as any).mapDeathCenter).toEqual(dc);

        // Self-check on the discriminator position: outside the
        // corrected circle, inside the buggy one.
        expect(Math.hypot(400 - dc.x, -800 - dc.y)).toBeGreaterThan(OUT_OF_BOUNDS_DISTANCE);
        expect(Math.abs(-800 - 5)).toBeLessThan(OUT_OF_BOUNDS_DISTANCE);

        const physics: any = (env as any).physics;
        physics.playerBodies.get(0).SetXForm({ x: 400 / SCALE, y: -800 / SCALE }, 0);
        env.step(0);
        const state = physics.getPlayerState(0);
        expect(state.alive).toBe(false);
        expect(state.deathType).toBe(4);
  ...
  describe('spawn-only / pure-cap-zone MapDef (bodies key omitted) survives normalizeMap (#273)', () => {
    const spawnOnlyMap = {
      ...
      capZones: [{ index: 0, owner: '', type: 1, fixture: 'plat_0', shapeType: 'rect' }],
      joints: [{ type: 'rv', name: 'j0', bodyA: 'plat_0', bodyB: 'plat_1' }],
