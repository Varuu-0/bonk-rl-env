...
describe('WorkerPool numOpponents validation (#392)', () => {
    it('rejects an out-of-range numOpponents before spawning workers, and the pool stays usable', async () => {
        ...
        try {
            await expect(
                pool.init(1, { maxTicks: 5, seed: 1, numOpponents: 87 }, true),
            ).rejects.toThrow(/Invalid numOpponents 87: expected at most 64 opponents/);
            await expect(
                pool.init(1, { maxTicks: 5, seed: 1, numOpponents: 87 }, true),
            ).rejects.not.toThrow(/reading 'next'/);
        ...
        for (const useSharedMemory of [true, false]) {
            ...
            try {
                await expect(
                    pool.init(
                        1,
                        { maxTicks: 5, seed: 1, num_opponents: MAX_OPPONENTS + 1 },
                        useSharedMemory,
                    ),
                ).rejects.toThrow(
