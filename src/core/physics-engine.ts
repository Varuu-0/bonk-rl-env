...
export class PhysicsEngine {
  ...
  /**
   * Process the deferred P3b respawns queued by the previous tick's death
   * pass (issue #339): return each queued disc to its spawn with the native
   * respawn mechanics (teleport, cleared grapple, fresh velocity). Idempotent
   * within a tick — a drain leaves `pendingRespawns` empty, so the second of
   * two back-to-back calls is a no-op.
   *
   * tick() calls this at its start so direct engine drivers keep their
   * contract; BonkEnvironment.step() additionally calls it BEFORE applying
   * inputs (#409) so a disc is already alive when applyInput runs — otherwise
   * the action sent for the tick in which the respawned disc becomes visible
   * is silently dropped by applyInput's alive-guard.
   */
  ...
  tick(): void {
    ...
    // tick after the death instead of inside the same pass. The
    // environment drains these earlier — see processRespawns (#409).
