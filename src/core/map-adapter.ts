...
function isInternalMapDef(raw: any): raw is MapDef {
  return (
    ...
    (raw.bodies === undefined ||
      raw.bodies === null ||
      raw.bodies.every(
        (b: any) => b === null || b === undefined || (typeof b === 'object' && !('collidesGroup1' in b)),
      ))
  );
}
...
function flattenNativeBodies(map: ExportedMap, nativeBodies: NativeBody[]): FlatBody[] {
  ...
  nativeBodies.forEach((nativeBody, bodyArrayIndex) => {
    ...
    const fixtureIndexes =
      nativeBody.fixtureIndices && nativeBody.fixtureIndices.length > 0
...
function toBodyDef(
  ...
  const type =
    body.type === 'rect' || body.type === 'circle' || body.type === 'polygon' ? body.type : nativeShapeType(shape);
  ...
  const worldCenter =
    fixtureCenter && nativeBody ? rotatePoint(fixtureCenter.x, fixtureCenter.y, nativeBody.angle ?? 0) : undefined;
  return {
    name,
    type,
    ...
    fricPolarity: body.fricPlayers === true || body.fricPlayers === 2 || body.fricp === true,
...
export function normalizeMap(raw: unknown): MapDef {
  ...
  if (flatBodies) {
    ...
    const hasStructured =
      ...
      (Array.isArray(map.physicsFixtures) &&
        map.physicsFixtures.some((f) => {
          if (f === null || f === undefined) return false;
          return !!(f.shape || typeof f.shapeIndex === 'number');
        })) ||
      (Array.isArray(map.physicsShapes) &&
        map.physicsShapes.some((s) => {
    ...
    for (const b of flatSource) {
      ...
      // makes sense).
      if (b.x === undefined && b.position?.x === undefined && b.y === undefined && b.position?.y === undefined)
        continue;
  ...
  const withRenderShape = (body: FlatBody, index: number): FlatBody => {
    ...
    const sourceBody =
      typeof body.bodyIndex === 'number' ? sourceBodies[body.bodyIndex] : sourceBodiesByFlatPosition[index];
  ...
  const source = flatSource.length > 0 ? renderSource : flattenNativeBodies(map, nativeBodies);
  ...
  let redSpawn =
    redCandidates.find((sp) => sp !== blueSpawn) ||
    redCandidates[0] ||
  ...
  source.forEach((b, i) => {
    ...
    const bodyIndex = nativeBody?.index ?? b.bodyIndex ?? i;
  ...
  // referents.
  const joints = (map.physicsJoints || [])
    .map((j, jointIdx) => {
      ...
      const bodyB = j.bodyB !== undefined && j.bodyB >= 0 ? bodiesByName.get(j.bodyB) : undefined;
      const safeType = (t: string | undefined): string => (t === undefined ? 'rv' : t);
      ...
      if (j.type === 'g' || safeType(j.type) === 'g') {
        ...
        if (!out.bodyA && refA && refA.bodyA !== undefined) {
          out.bodyA =
            refA.bodyA >= 0 ? (bodiesByName.get(refA.bodyA) ?? '') : refA.bodyA === -1 ? GROUND_BODY_NAME : '';
        }
        if (!out.bodyB && refB && refB.bodyB !== undefined) {
          out.bodyB =
            refB.bodyB >= 0 ? (bodiesByName.get(refB.bodyB) ?? '') : refB.bodyB === -1 ? GROUND_BODY_NAME : '';
        }
      }
      return out;
    })
  ...
  if (bodies.length > 0) {
    ...
    for (const b of bodies) {
      let bx0 = b.x,
        ...
        by0 = b.y,
        by1 = b.y;
      if (b.type === 'circle' && b.radius) {
        bx0 = b.x - b.radius;
        bx1 = b.x + b.radius;
        by0 = b.y - b.radius;
        by1 = b.y + b.radius;
      } else if (b.type === 'rect' && b.width && b.height) {
        // addBody() builds rects as oriented boxes rotated by
        ...
        const hh = (b.width / 2) * sinA + (b.height / 2) * cosA;
        bx0 = b.x - hw;
        bx1 = b.x + hw;
        by0 = b.y - hh;
        by1 = b.y + hh;
  ...
  const physics =
    (ph as any).ppm !== undefined || bounds.width !== undefined || deathCenter || typeof (ph as any).nc === 'boolean'
      ? {
          ppm: (ph as any).ppm,
  ...
  return {
    ...
    bodyRenderOrder: Array.isArray((map as any).bodyRenderOrder)
      ? (map as any).bodyRenderOrder.filter(
          (index: unknown): index is number => typeof index === 'number' && Number.isInteger(index) && index >= 0,
        )
      : undefined,
