  ...
  try {
    if (msg.type === 'init') {
      ...
      for (let i = 0; i < numEnvsParam; i++) {
        ...
        // is now honored, #200) plus the global env index, wrapped
        // into the supported seed domain (#460) so a valid-but-high
        // configured seed with several environments cannot derive an
        // out-of-domain seed that the constructor guard would reject.
        // Pooled environments therefore never all share an identical
        // PRNG stream, while every environment's construction stream
        // stays reproducible for the same config. The worker's later
        // reset() with per-env seeds overrides this stream exactly as
        // before.
        envs.push(new BonkEnvironment({ ...config, seed: deriveConstructionSeed(config.seed ?? 0, globalOffset + i) }));
      }
      ...
      console.log(
        `[Worker] Init: sharedBuffer valid = ${isValidSharedBuffer}, byteLength = ${msg.sharedBuffer?.byteLength}`,
      );
      ...
      if (sharedMem) {
        results.forEach((res, i) => {
          ...
          sharedMem!.writeObservation(
            i,
            res.done ? observationToArray(res.observation) : observationFastToArray(envs[i]),
          );
